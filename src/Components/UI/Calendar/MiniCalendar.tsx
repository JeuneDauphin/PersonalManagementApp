...
const MiniCalendar: React.FC<MiniCalendarProps> = ({ value, onChange, onClose, minDate, className }) => {
  ...
  return (
    <div ref={wrapperRef} className={`absolute z-50 mt-2 bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-3 w-72 ${className || ''}`}>
        ...
        {days.map(d => {
          ...
          return (
            <button
              key={d.toISOString()}
              disabled={disabled}
              onClick={() => { onChange(d); onClose && onClose(); }}
